...
class Login_attempts extends CI_Model
{
	...
	function get_attempts_num($ip_address, $login)
	{
		...
		$this->db->where('la_ip_address', $ip_address);
		if (strlen($login) > 0) $this->db->or_where('la_login', $login);
	...
	{
		$this->db->insert($this->table_name, array('la_ip_address' => $ip_address, 'la_login' => $login));
	}
	...
	{
		$this->db->where(array('la_ip_address' => $ip_address, 'la_login' => $login));

		// Purge obsolete login attempts
		//$this->db->or_where('UNIX_TIMESTAMP(time) <', time() - $expire_period); 
		$this->db->or_where( 'ROUND(DATE_PART(\'epoch\', la_time)) <', time() - $expire_period); // postgresql equivalent
...
/* Location: ./application/models/admin/auth/login_attempts.php */